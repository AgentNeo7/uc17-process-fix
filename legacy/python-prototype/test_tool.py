...
class ToolTests(unittest.TestCase):
    ...
    def test_unknown_category_kept(self):
        self.data['events'][0]['category']='unknown'
        self.assertTrue(any(h['category']=='unknown' for h in tool.analyze(self.payload)['hypotheses']))
    def test_zero_exposure_not_zero_rate(self):
        self.data['before']={'exposure':0,'failures':0}
        self.assertIsNone(tool.analyze(self.payload)['recurrence']['before_rate'])
    def test_impossible_counts_rejected(self):
        self.data['after']={'exposure':1,'failures':2}
